...
import { Activity, Video, Database, Zap, Cpu, Server, ShieldCheck, FileText, ArrowRight, Layers, Radio } from 'lucide-react';
...
const PipelineInspector = () => {
  ...
  const { playHover, playClick, playDigital } = useSoundEffects();

  const currentPipeline = pipelines[activePipelineIndex];
  ...
  return (
    <section className="pipeline-section section-padding border-bottom" id="architecture">
      <div className="container">
        
        {/* Header */}
        <div className="section-header-row" style={{ marginBottom: '2.5rem' }}>
          <div>
            <span className="section-eyebrow mono-text">SYSTEM ARCHITECTURE</span>
            <h2 className="section-title">Data Pipeline Inspector</h2>
          </div>
          <p className="section-subtitle">
            Interactive node-by-node telemetry and latency budgets powering Adithyan's distributed systems.
          </p>
        </div>
        ...
        <div className="pipeline-tabs-wrapper">
          {pipelines.map((p, idx) => (
            <button
              key={p.id}
              className={`pipeline-tab-btn ${activePipelineIndex === idx ? 'active-tab' : ''}`}
              onClick={() => handleTabChange(idx)}
              onMouseEnter={playHover}
              type="button"
            >
              <Radio size={14} className={activePipelineIndex === idx ? 'tab-radio-active' : ''} />
              <span className="mono-text">{p.badge}</span>
            </button>
          ))}
        </div>

        {/* Main Pipeline Box */}
        <div className={`pipeline-box liquid-glass ${currentPipeline.stripeClass}`}>
