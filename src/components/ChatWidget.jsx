  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      if (contentType.includes('application/json')) {
        ...
        throw new Error(
          response.status === 404
            ? 'Endpoint /api/chat not found. If running locally with Vite, please restart "npm run dev".'
            : `Server returned ${response.status}: ${textResp.slice(0, 100)}`
        );
      }
      ...
      if (!replyContent) {
        throw new Error('No reply generated by the model. Please try again.');
      }
